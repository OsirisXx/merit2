...
<head>
    ...
    <title>History Dashboard - Ally</title>
...
<body>
    <div class="container">
        ...
        <main class="main-content">
            <h2 id="page-title">Admin History Overview</h2>
                ...
                <div class="cards-container">
                    <div class="card" id="donation-card">
                        ...
                        <div class="card-message">
                            Currently: <span id="donation-count">0</span> donations completed
                        </div>
                    </div>
                    ...
                    <div class="card" id="matching-card">
                        ...
                        <div class="card-message">
                            Currently: <span id="matching-count">0</span> matches processed
                        </div>
                    </div>
                    ...
                    <div class="card" id="appointment-card">
                        ...
                        <div class="card-message">
                            Currently: <span id="appointment-count">0</span> appointments held
                        </div>
                    </div>
                </div>
            </div>
    ...
    <script>
        ...
        // --- Fetch and Display Donation History (matching mobile app logic) ---
        async function fetchAndDisplayDonations(filterType = 'All Donations') {
            ...
            donationNoRecordsMsg.style.display = 'none';
                    ...
                    // Multiple donation collections to check (matching mobile app)
        ...
        // --- Fetch and Display Adoption Progress History (matching mobile app logic) ---
        async function fetchAndDisplayAdoptionProgress(filterType = 'All Adoptions') {
            ...
            try {
                ...
                snapshot.forEach(doc => {
                    ...
                    // Add all completed adoptions from this document
        ...
        // --- Fetch and Display Matching History (matching mobile app logic) ---
        async function fetchAndDisplayMatchingHistory(filterType = 'All Matches') {
            ...
            try {
                // 1. Load successful matches (accepted/matched/completed) from matching_preferences
                const successfulQuery = db.collection('matching_preferences');
                const successfulSnapshot = await successfulQuery.get();
                ...
                // 2. Load previous match history from matching_history collection
        ...
        // --- Fetch and Display Appointment History (matching mobile app logic) ---
        async function fetchAndDisplayAppointmentHistory(filterType = 'All Appointments') {
            ...
            try {
                ...
                snapshot.forEach(doc => {
                    ...
                    // Only show completed and cancelled appointments (matching mobile app logic)
                    ...
                    if (filterType !== 'All Appointments') {
                        ...
                        if (filterType === 'Cancelled' && status !== 'cancelled') return;
                    }

                    // Use completion/cancellation timestamp or scheduled timestamp
                    const timestamp = appointment.completedAt || appointment.cancelledAt || appointment.scheduledTimestamp;
        ...
        // --- Count Functions (matching mobile app logic) ---
        async function loadDonationCounts() {
            try {
        ...
        async function loadMatchingCounts() {
            try {
                let totalMatches = 0;
                ...
                if (user) {
                    try {
                        ...
                        if (userDoc.exists && userDoc.data().role === 'admin') {
                            ...
                            displayAdminStatusMessage('Admin history dashboard loaded.', 'success');

                            // Load counts matching mobile app logic
                            loadDonationCounts();
                            loadAdoptionCounts();
                            loadMatchingCounts();
                            ...
                            if (adminContentDiv) adminContentDiv.innerHTML = '<p style="text-align: center; margin-top: 50px; font-size: 1.2em;">You do not have permission to view this page. Please log in with an administrator account.</p>';
                            if (adminContentDiv) adminContentDiv.style.display = 'block';
                        }
                    } catch (error) {
                    ...
                    displayAdminStatusMessage('Please sign in to access the history dashboard.', 'error');
                    if (adminContentDiv) adminContentDiv.innerHTML = '<p style="text-align: center; margin-top: 50px; font-size: 1.2em;">Please <a href="signin.php">sign in</a> to access the history dashboard.</p>';
                    if (adminContentDiv) adminContentDiv.style.display = 'block';
                }
            });
        });
